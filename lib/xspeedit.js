 ...
 * @param articlesStr chaine d'articles
 * @param callback callback de succès
 * @err callback d'erreur
 */
var XspeedIt = function (articlesStr, callback, err) {
    var that = this;
    ...
    process.nextTick(function () {
        ...
        if (articlesStr.length == 0) {
            err(new Error('Il faut au moins un article.'));
        }
        try {
...
module.exports = function (articles, callback, err) {
    new XspeedIt(articles, callback, err);
}
